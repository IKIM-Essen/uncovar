...
def plot_error(sm_input, sm_output):
    ...
    scatter = alt.Chart(results_df).mark_point().encode(
        x='true_fraction:Q',
        y='est_fraction:Q',
        color= "Kallisto output:N"
    )

    line = alt.Chart(
    pd.DataFrame({'true_fraction': [0, 1], 'est_fraction': [0, 1]})).mark_line().encode(
            alt.X('true_fraction'),
            alt.Y('est_fraction')
    )
    
    (scatter+line).properties(
        title=f"No. of mixtures {no_of_mixs}"
    ).save(sm_output)
