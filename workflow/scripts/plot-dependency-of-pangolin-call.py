...
def plot_dependency_of_pangolin_call(sm_input, sm_output):
    ...
    for input in sm_input:
        ...
        pangolin_output["mixture_content"] = input.split(MIXTURE_PREFIX, 1)[-1].split(".")[0]
    ...
    all_sampes["regex"] = MIXTURE_PART_INDICATOR + all_sampes["lineage"] + MIXTURE_PERCENTAGE_INDICATOR + '(?P<share>\d.)'
    all_sampes['share_of_called_lineage'] = all_sampes[['mixture_content','regex']].apply(lambda x: regex_per_line(*x), axis=1)
    all_sampes.drop(columns=["regex"], inplace=True)
    all_sampes["correct_call"] = all_sampes['share_of_called_lineage'].apply(lambda x: "Yes" if x > 0 else "No")
    ...
    histogram = alt.Chart(source).mark_bar().encode(
        alt.X("Share of called lineage in sample:Q", bin=alt.Bin(extent=[0, 1], step=0.1), axis=alt.Axis(format='%')),
        y='count()',
        color=alt.Color("Lineage in sample", scale=alt.Scale(scheme="tableau20"), sort="descending"),
    ).configure_legend(
        orient='top'
    )
