...
for path, sample, stage in zip(snakemake.input, snakemake.params.samples, snakemake.params.stages):
    pangolin_call = pd.read_csv(path)
    pangolin_call["sample"] = sample
    pangolin_call["stage"] = stage
    
    pangolin_calls.append(
        pangolin_call
    )

pangolin_calls_by_stage = pd.concat(pangolin_calls, axis=0, ignore_index=True)
pangolin_calls_by_stage = pangolin_calls_by_stage.pivot(index="sample", columns="stage", values='lineage').reset_index()

only_pseudo_assembly = ["scaffold","polished","masked-polished","pseudo"]
only_consensus_assembly = ["scaffold","polished","masked-polished","consensus","masked-consensus"]
both_fallbacks= list(set(only_pseudo_assembly).symmetric_difference(set(only_consensus_assembly)))

columns = pangolin_calls_by_stage.columns.to_list()

if all(stage in columns for stage in both_fallbacks):
    sorted_columns = ["sample", "scaffold", "polished", "masked-polished", "consensus", "masked-consensus", "pseudo"]
    ...
    sorted_columns = ["sample", "scaffold", "polished", "masked-polished", "consensus", "masked-consensus"]

pangolin_calls_by_stage = pangolin_calls_by_stage[sorted_columns]

pangolin_calls_by_stage.rename(columns={
    "sample" : "Sample",
