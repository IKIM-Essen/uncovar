...
# add SEQ_TYPE
illumina_samples = pd.DataFrame.from_dict({key:val for d in snakemake.params[0] for key,val in d.items()}, orient="index", columns=["SEQ_TYPE"])
csv_table = csv_table.merge(illumina_samples, how="left", left_on="OWN_FASTA_ID", right_index = True)
