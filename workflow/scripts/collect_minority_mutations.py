...
def bcf_to_df(data_dict, AA_ALPHABET_TRANSLATION):
    ...
    for file in snakemake.input.bcfs:
        ...
        lineage = (
            lineages["Scaffolded Seq"].values[idx]
            if lineages["Scaffolded Seq"].values[idx] != "Not called on"
            else lineages["Pseudo Seq"].values[idx]
        )
            ...
            for ann in record.info["ANN"]:
                ...
                if pattern_count == 0 and alt != "":
                    ...
                    for triplet, amino in AA_ALPHABET_TRANSLATION.items():
                        alteration = alteration.replace("%3D", triplet[0]).replace(
                            triplet, amino
                        )
                    ...
                    raise Exception(
                        "Something unexpected occured in this record:",
                        str(record),
                        file,
                    )
                ...
                # probability
                data_dict["Probability"].append(
                    1.0
                    - (
                        phred_to_prob(record.info["PROB_ABSENT"][0])
