...
covariants_data = requests.get("https://github.com/hodcroftlab/covariants/blob/master/web/data/clusters.json").json()
...
class SynonymousVariant:
    ...
    def __eq__(self, other):
        return self.left == other.left and self.right == other.right and self.pos == other.pos
...
with FastaFile(snakemake.input.reference) as infasta:
    ...
    header.add_line('INFO=<ID=SIGNATURES,Number=.,Type=String,Description="Variant signature as obtained from covariants.org">')
    header.add_line('INFO=<ID=LINEAGES,Number=.,Type=String,Description="Lineages having this variant">')

    NonSynonymousVariant = namedtuple("NonSynonymousVariant", ["gene", "left", "pos", "right"])
    ...
    for lineage_entry in covariants_data["clusters"]:
        for variant in lineage_entry["mutations"]["nonsynonymous"]:
            known_non_synonymous_variants[NonSynonymousVariant(**variant)].add(lineage_entry["build_name"])

    known_synonymous_variants = defaultdict(set)
    for lineage_entry in covariants_data["clusters"]:
        for variant in lineage_entry["mutations"]["synonymous"]:
            known_synonymous_variants[SynonymousVariant(**variant)].add(lineage_entry["build_name"])

    with VariantFile(snakemake.output[0], "wb", header=header) as outvcf:
        def get_variants(all_variants, variant_type, merge=True):
            filtered_variants = sorted(filter(lambda item: item[0].variant_type() == variant_type, all_variants.items()))
            ...
            def process_batch(batch, batch_lineages):
                ...
                all_lineages = np.array(list(set(lineage for lineages in batch_lineages for lineage in lineages)))
                # Step 2: build matrix of variants vs lineages (columns mark combinations of variants that can be merged)
                lineage_matrix = np.array([[(lineage in lineages) for lineage in all_lineages] for lineages in batch_lineages])
            ...
            for variant, lineages in filtered_variants:
                if variant.pos == batch[-1].pos + 1 and variant.is_same_feature(batch[-1]):
        ...
        for variants, lineages in get_variants(known_synonymous_variants, VariantType.Ins):
        ...
        for variants, lineages in get_variants(known_synonymous_variants, VariantType.Del):
            pos = variants[0].genome_pos() - 1
        ...
        for variant, lineages in get_variants(known_synonymous_variants, VariantType.Subst, merge=False):
            pos = variants[0].genome_pos()
            write_record(pos, variant.left, variant.right, lineages, [variant])

        for variants, lineages in get_variants(known_non_synonymous_variants, VariantType.Ins):
            pos = variants[0].genome_pos() - 1
            ref_allele = infasta.fetch(reference=contig, start=pos, end=pos + 1)
        ...
        for variants, lineages in get_variants(known_non_synonymous_variants, VariantType.Del):
            pos = variants[0].genome_pos() - 1
            alt_allele = infasta.fetch(reference=contig, start=pos, end=pos + 1)
            ref_allele = alt_allele + aa_to_dna("".join(variant.left for variant in variants))
            write_record(pos, ref_allele, alt_allele, lineages, variants)

        for variant, lineages in get_variants(known_non_synonymous_variants, VariantType.Subst, merge=False):
