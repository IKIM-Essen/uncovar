...
def plot_lineages_over_time(sm_input, sm_output, dates):
    ...
    # get occurrences
    pangolin_calls['lineage_count'] = pangolin_calls.groupby('lineage', as_index=False)['lineage'].transform(lambda s: s.count())

    # mask low occurrences
    pangolin_calls.loc[pangolin_calls["lineage_count"] < 10, "lineage"]="other (< 10 occ.)"
    ...
    area_plot = alt.Chart(source).mark_area(opacity=0.5, interpolate='monotone').encode(
