...
def analyize_pangolin(sm_input):
    ...
    for pango_csv_path in sm_input.pangolin:
        sample = pango_csv_path.replace("results/test-cases/tables/strain-calls/", "").replace(".strains.pangolin.csv", "").replace("non-cov2-", "")
        with open(pango_csv_path, "r") as pango_file:
            ...
            if pango_df.loc[0, "status"] == "fail":
                ...
                if pango_df.loc[0, "status"] == "pass" and pango_df.loc[0, "lineage"] == "None":
                    temp_dict[sample] = "is non-sars-cov-2"
                else:
...
def analyize_kallisto(sm_input):
    ...
    for kallisto_csv_path in sm_input.kallisto:
        sample = kallisto_csv_path.replace("results/test-cases/tables/strain-calls/", "").replace(".strains.kallisto.tsv", "").replace("non-cov2-", "")
...
def aggregate_and_save(pangolin_summary, kallisto_summary, sm_output):
    agg_df = pd.DataFrame([pangolin_summary, kallisto_summary], index=["pangolin", "kallisto"]).T
