...
score_thresholds["PolyPhen"] = {
  Benign: [0, 0.149],
  "Possibly Damaging": [0.15, 0.849],
  "Probably Damaging": [0.85, 1],
};
score_thresholds["SIFT"] = { Benign: [0, 0.05], Damaging: [0.051, 1] };

let score_scales = {};
score_scales["SIFT"] = {
  colors: ["#2ba6cb", "#ff5555"],
  entries: ["Benign", "Damaging"],
};
score_scales["PolyPhen"] = {
  colors: ["#2ba6cb", "#ffa3a3", "#ff5555"],
  entries: ["Benign", "Possibly Damaging", "Probably Damaging"],
};
...
$(document).ready(function () {
  $("html").on("click", ".variant-row", function () {
    ...
    $("#ann-sidebar").append(
      '<th class="thead-dark" style="position: sticky; left:-1px; z-index: 1; background: white">Linkouts</th>'
    );
    var sift_scores = [];
    var polyphen_scores = [];
    for (let j = 1; j <= ann_length; j++) {
      ...
      $("#Div" + j).append(
        '<a class="btn btn-secondary dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Select source</a>'
      );
      $("#Div" + j).append(
        '<div id="Button' +
          j +
      ...
      $("#Button" + j).append(
        '<a class="dropdown-item" href="https://clinicaltrials.gov/ct2/results?cond=&term=' +
          gene +
          '&cntry=&state=&city=&dist=" target="_blank">ClinicalTrials</a>'
      );
      $("#Button" + j).append(
        '<a class="dropdown-item" href="https://www.cbioportal.org/ln?q=' +
          gene +
          '" target="_blank">cBioPortal</a>'
      );
      $("#Button" + j).append(
        '<a class="dropdown-item" href="https://oncokb.org/gene/' +
          gene +
          '" target="_blank">OncoKB</a>'
      );
      $("#Button" + j).append(
        '<a class="dropdown-item" href="https://www.ensembl.org/homo_sapiens/Gene/Summary?db=core;g=' +
          ensembl_id +
          '" target="_blank">Ensembl</a>'
      );
      $("#Button" + j).append(
        '<a class="dropdown-item" href="https://whi.color.com/gene/' +
          ensembl_id +
          '" target="_blank">FLOSSIES</a>'
      );
      $("#Button" + j).append(
        '<a class="dropdown-item" href="https://varsome.com/gene/' +
          gene +
    ...
    $.each($(this).data("info"), function (key, value) {
      if (key.startsWith("PROB_")) {
        ...
        prob_type =
          prob_type.charAt(0) +
          prob_type.substring(1).toLowerCase().replace("_", " ");
    ...
    $.each(
      $(this).data("format")["AF"],
      function (sample_name, allele_frequency) {
        binom_dist = binom_dist.concat(
          calcBinomDist(
            parseFloat(allele_frequency),
            af[sample_name][0],
            sample_name
          )
        );
    ...
    function calcBinomDist(alleleFreq, coverage, sample_name) {
      ...
      while (frequency <= 1) {
        var value =
          math.combinations(coverage, k) *
          Math.pow(frequency, k) *
          Math.pow(1 - frequency, coverage - k);
        binomValues.push({
          frequency: frequency,
          binomProb: value,
          sample: sample_name,
    ...
    function parse_score(scores, variant_value, score_type, transcript) {
      ...
      for (bin of effect_bins) {
        var effect_idx = score_scales[score_type]["entries"].indexOf(
          bin.effect
        );
        scores.push({
          name: score_type,
