...
with open("/local/work/uncovar-wastewater/results/2022-12-21/ampliconstats/D01.ampliconstats.txt", "r") as statfile:
...
amplicon_transpose = amplicon_profiles.copy(deep=True)
# amplicon_transpose.reset_index(drop=False, inplace=True)

amplicon_transpose.iloc[0,0] = "no_amplicon"

amplicon_transpose = amplicon_transpose.transpose()

amplicon_transpose.replace({"", np.NaN}, inplace=True)
print("pivot")
print(amplicon_transpose)

for column in amplicon_transpose.columns:
    print(f"amplicon {column}\n{amplicon_transpose[column].value_counts(ascending=False ,dropna=True).index.to_list()}")
...
filter_data = data[(data["Probability"] > 0.9) & (data["Frequency"] > 0.5) & (data["ReadDepth"] > 10)].copy(deep=True)

filter_data.reset_index(inplace=True, drop=False)
filter_data = filter_data[["mutation", "amplicon"]]
filter_data = filter_data.groupby(["amplicon"]).agg(lambda x: ' '.join(set(x)))
...
for index, mutation_profile in filter_data.iterrows():
    ...
    print(index  + "\t" +  mutation_profile["mutation"], lineage_hit)
