...
for rec in bcf_in.fetch():
    data = pd.concat(
        [data, pd.DataFrame
            (
                {
                    "snv-combi": ", ".join(rec.info["SIGNATURES"]),
                    "snv": rec.info["SIGNATURES"],
                    "position": rec.pos,
                    "lineage": ",".join(rec.info["LINEAGES"])
                },
            ),
        ],ignore_index=True
    )
...
print(data)
#                     "lineage": ",".join([lin.replace(".", " ") for lin in rec.info["LINEAGES"]])
...
amplicons["number"] = amplicons.index.to_series() + 1
amplicons.index = pd.IntervalIndex.from_arrays(amplicons["amp_start"], amplicons["amp_end"], closed="both")
...
data["pango"] = data["lineage"].apply(lambda x: ",".join(rename_dict[y] if y in rename_dict else y for y in x.split(",")))

data["amplicon"] = data["position"].apply(lambda x: amplicons.loc[amplicons.index.contains(x)]["number"].values)
data = data.explode("amplicon")
data.reset_index(inplace=True, drop=True)

print(data)

data.to_csv("test_amp_interval.csv")
