...
def register_contig_lengths(assemblies_init, assemblies_final, N50, assembler, data):
    for sample, amplicon_state, file1, file2, quast in zip(
        snakemake.params.samples,
        snakemake.params.amplicon_state,
        assemblies_init,
        assemblies_final,
        N50,
    ):
...
for assembler in dict_init:
    data = register_contig_lengths(
        dict_init[assembler],
        dict_final[assembler],
        dict_N50[assembler],
        assembler,
        data,
    )
...
combined_bp = plot_bp.mark_boxplot(opacity=0.5) + plot_bp.mark_point(
