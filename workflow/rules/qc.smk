...
rule extract_unmapped:
    ...
    shell:
        ...
        samtools sort  -@ {threads} -n -o {output.t1} {output.t1} 2>&1 >> {log} 
        samtools sort  -@ {threads} -n -o {output.t2} {output.t2} 2>&1 >> {log}
        samtools sort  -@ {threads} -n -o {output.t3} {output.t3} 2>&1 >> {log}
        samtools merge -@ {threads} -n {output.unsorted} {output.t1} {output.t2} {output.t3} 2>&1 >> {log}
        samtools sort  -@ {threads} -n {output.unsorted} -o {output.sorted} 2>&1 >> {log}
        samtools fastq -@ {threads} {output.sorted} -1 {output.fq1} -2 {output.fq2} 2>&1 >> {log}
        """
